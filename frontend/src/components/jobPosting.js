...
function PostJob() {
  ...
  const handleSubmit = async (e) => {
    ...
    const areAllEmailsValid = formData.candidates.every((candidate) => isValidEmail(candidate.email));
    const areAllFieldsFilled = formData.title && formData.description && formData.endDate;
    const areCandidatesPresent = formData.candidates.length > 0;
    ...
    try {
      ...
      // Reset fields if the post was successful
      if (response.ok) {
        setFormData({
          title: '',
          description: '',
    ...
    <Container>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        {/* {formData.candidates.length === 0 && (
          <Message>Please add at least one candidate.</Message>
        )} */}
        {formData.candidates.map((candidate, index) => (
          <CandidateField key={index}>
            <Input
